...
import os, glob
from flex.tools.configurations import load_config
import torch
...
def exprdir2model(expr_dir):
    ...
    assert len(available_ckpts) > 0, ValueError('No check points found at {}'.format(model_snapshots_dir))
...
def load_model(expr_dir, model_code=None, remove_words_in_model_weights=None, load_only_ps=False, disable_grad=True, device='cpu', custom_ps=None):
    '''
    ...
    state_dict = torch.load(trained_weigths_fname, map_location=device)['state_dict']
